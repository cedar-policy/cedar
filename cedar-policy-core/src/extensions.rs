...
impl<'a> Extensions<'a> {
    ...
    pub fn func(&self, name: &Name) -> Result<&ExtensionFunction> {
        ...
        match extension_funcs.get(0) {
            None => Err(FailedExtensionFunctionLookup::FuncDoesNotExist { name: name.clone() }),
            Some(first) if extension_funcs.len() == 1 => Ok(first),
            _ => Err(FailedExtensionFunctionLookup::FuncMultiplyDefined {
                name: name.clone(),
    ...
    pub(crate) fn lookup_single_arg_constructor(
        ...
        match matches.get(0) {
            ...
            _ => Err(
                FailedExtensionFunctionLookup::MultipleConstructorsSameSignature {
...
pub enum FailedExtensionFunctionLookup {
...
pub type Result<T> = std::result::Result<T, FailedExtensionFunctionLookup>;
