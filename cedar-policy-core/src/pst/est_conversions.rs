...
impl TryFrom<est::Expr> for Expr {
    ...
    fn try_from(est_expr: est::Expr) -> Result<Self, Self::Error> {
        match est_expr {
...
impl TryFrom<est::ExprNoExt> for Expr {
    ...
    fn try_from(est_expr: est::ExprNoExt) -> Result<Self, Self::Error> {
        ...
        match est_expr {
            ...
            E::IsEmpty { arg } => Ok(Expr::is_empty(Arc::unwrap_or_clone(arg).try_into()?)),
...
mod tests {
    ...
    fn test_est_expr_not() {
        ...
        let est_expr: est::Expr = serde_json::from_str(json).unwrap();
        let pst_expr: Expr = est_expr.try_into().unwrap();
        assert!(matches!(pst_expr, Expr::Not(_)));
    }
    ...
    fn test_est_expr_neg() {
        ...
        let est_expr: est::Expr = serde_json::from_str(json).unwrap();
        let pst_expr: Expr = est_expr.try_into().unwrap();
        assert!(matches!(pst_expr, Expr::Neg(_)));
    }
    ...
    fn test_est_to_pst_is_empty() {
        ...
        let pst_expr: Expr = est_expr.try_into().unwrap();
        assert!(matches!(pst_expr, Expr::IsEmpty(_)));
    }
    ...
    fn test_est_expr_func_call() {
        let test_cases = vec![
            (
                "single_arg",
                r#"{"decimal": [{"Value": "1.23"}]}"#,
                "decimal",
                1,
            ),
            (
                "multiple_args",
                r#"{"ip": [{"Value": "192.168.0.1"}, {"Value": "255.255.255.0"}]}"#,
                "ip",
                2,
            ),
        ...
        for (test, test_json, expect_funcname, expect_arg_len) in test_cases {
            ...
            if let Expr::FuncCall { name, args } = pst_expr.try_into().unwrap() {
                ...
                assert_eq!(
                    args.len(),
                    expect_arg_len,
                    "wrong arguments length in {}",
                    test
                );
            } else {
                panic!("Expected FuncCall in {}", test);
            }
        }
        ...
        if let Expr::FuncCall { name, args } = est_expr.try_into().unwrap() {
            ...
            panic!("Expected FuncCall with complex arg");
        }
        ...
        if let Expr::FuncCall { name, args } = est_expr.try_into().unwrap() {
            ...
            panic!("Expected FuncCall with nested call");
        }
    }
