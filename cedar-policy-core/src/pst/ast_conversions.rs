...
use super::expr::{BinaryOp, EntityUID, Expr, Literal, PatternElem, Var};
...
fn expr_to_ast(expr: impl AsRef<Expr>) -> Result<ast::Expr, ConversionError> {
    ...
    match expr.as_ref() {
        ...
        // Unary operations
        Expr::Not(e) => Ok(builder.not(expr_to_ast(e.clone())?)),
        Expr::Neg(e) => Ok(builder.neg(expr_to_ast(e.clone())?)),
        ...
        Expr::BinaryOp { op, left, right } => {
            ...
            match op {
                BinaryOp::Eq => Ok(builder.is_eq(left_ast, right_ast)),
                BinaryOp::NotEq => Ok(builder.noteq(left_ast, right_ast)),
                BinaryOp::Less => Ok(builder.less(left_ast, right_ast)),
                BinaryOp::LessEq => Ok(builder.lesseq(left_ast, right_ast)),
                BinaryOp::Greater => Ok(builder.greater(left_ast, right_ast)),
                BinaryOp::GreaterEq => Ok(builder.greatereq(left_ast, right_ast)),
                BinaryOp::And => Ok(builder.and(left_ast, right_ast)),
                BinaryOp::Or => Ok(builder.or(left_ast, right_ast)),
                BinaryOp::Add => Ok(builder.add(left_ast, right_ast)),
                BinaryOp::Sub => Ok(builder.sub(left_ast, right_ast)),
                BinaryOp::Mul => Ok(builder.mul(left_ast, right_ast)),
                BinaryOp::In => Ok(builder.is_in(left_ast, right_ast)),
                BinaryOp::Contains => Ok(builder.contains(left_ast, right_ast)),
                BinaryOp::ContainsAll => Ok(builder.contains_all(left_ast, right_ast)),
                BinaryOp::ContainsAny => Ok(builder.contains_any(left_ast, right_ast)),
                BinaryOp::GetTag => Ok(builder.get_tag(left_ast, right_ast)),
                BinaryOp::HasTag => Ok(builder.has_tag(left_ast, right_ast)),
            }
        }
        ...
        Expr::IsEmpty(e) => Ok(builder.is_empty(expr_to_ast(e.clone())?)),
        ...
        Expr::FuncCall { name, args } => Ok(builder.call_extension_fn(
            name.clone()
                .try_into()
                .map_err(|err| ConversionError::InvalidConversion(format!("{:?}", err)))?,
            args.into_iter()
                .map(|arg| expr_to_ast(arg))
