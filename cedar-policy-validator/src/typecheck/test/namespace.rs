...
use crate::{
    ...
    RawReservedName, SchemaError, SchemaFragment, ValidationWarning, ValidatorSchema,
};

fn namespaced_entity_type_schema() -> SchemaFragment<RawReservedName> {
    serde_json::from_str(
        r#"
            { "N::S": {
                "entityTypes": {
...
fn namespaced_entity_type_in_attribute() {
    let schema: SchemaFragment<RawReservedName> = serde_json::from_str(
        r#"{ "N::S":
            {
                "entityTypes": {
                    "Foo": {
...
fn namespaced_entity_type_member_of() {
    let schema: SchemaFragment<RawReservedName> = serde_json::from_value(serde_json::json!(
    {"N::S": {
        "entityTypes": {
            "Foo": {
...
fn multiple_namespaces_literals() {
    let authorization_model: SchemaFragment<RawReservedName> = serde_json::from_value(json!(
        {
            "A": {
                "entityTypes": {"Foo": {}},
                "actions": {}
