...
impl From<&cedar_policy_validator::ValidatorEntityType> for models::ValidatorEntityType {
    fn from(v: &cedar_policy_validator::ValidatorEntityType) -> Self {
        Self {
            name: Some(models::EntityType::from(v.name())),
            descendants: v.descendants.iter().map(models::EntityType::from).collect(),
            attributes: Some(models::Attributes::from(v.attributes())),
            open_attributes: models::OpenTag::from(&v.open_attributes()).into(),
            tags: v.tag_type().map(|tags| models::Tag {
...
impl From<&models::ValidatorEntityType> for cedar_policy_validator::ValidatorEntityType {
    ...
    fn from(v: &models::ValidatorEntityType) -> Self {
        Self::new(
            ast::EntityType::from(v.name.as_ref().expect("name field should exist")),
            v.descendants.iter().map(ast::EntityType::from),
            types::Attributes::from(
                v.attributes
                    .as_ref()
