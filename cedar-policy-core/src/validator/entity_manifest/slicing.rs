...
impl AccessPath {
    ...
    pub fn compute_value(
        ...
        match variant {
            ...
            AccessPathVariant::Literal(euid) => Ok(Value::from(Literal::EntityUID(std::sync::Arc::new(euid.clone())))),
                ...
                match var {
                    crate::ast::Var::Principal => {
                        if let Some(principal) = request.principal().uid() {
                            Ok(Value::from(Literal::EntityUID(std::sync::Arc::new(principal.clone()))))
                        } else {
                    ...
                    crate::ast::Var::Action => {
                        if let Some(action) = request.action().uid() {
                            Ok(Value::from(Literal::EntityUID(std::sync::Arc::new(action.clone()))))
                        } else {
                            Err(EntitySliceError::PartialRequest(PartialRequestError {}))
                        }
                    }
                    crate::ast::Var::Resource => {
                        if let Some(resource) = request.resource().uid() {
                            Ok(Value::from(Literal::EntityUID(std::sync::Arc::new(resource.clone()))))
                        } else {
                            Err(EntitySliceError::PartialRequest(PartialRequestError {}))
                        }
                    }
