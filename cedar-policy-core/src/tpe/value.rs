...
impl PartialRecord {
    ...
    pub(crate) fn attrs(&self) -> impl Iterator<Item = (&SmolStr, &PartialAttribute)> {
        self.0.iter()
    }

    /// Get the partial value of `attr`, informed by record's declared attribute types
    ///
    ...
    ///
    /// Errors if `action` is undeclared, or if the context supplies an attribute its context type
    /// does not declare. This is not typechecking the context, but it needs to know what attribute
    /// may exist to construct a fully concrete (partial) value.
    pub fn concrete_context_for_action(
        map: &BTreeMap<SmolStr, Value>,
        action: &EntityUID,
        schema: &ValidatorSchema,
    ) -> Result<Self, UntypedValueError> {
        let Some(Type::Record { attrs, .. }) =
            schema.get_action_id(action).map(|a| a.context_type())
        else {
            return Err(UntypedValueError::UntypedContext(action.clone()));
        };
        Self::from_concrete_map(map, attrs).map_err(UntypedValueError::UntypedAttrOrTag)
    }

    /// Construct from a concrete attributes map and its type
    ///
    /// Because we're converting from a concrete map, we never create a `PartialAttribute::Unknown`.
    ...
    ///
    /// Returns `None` if any field is `Exists` (unknown value), if a `Value` field is not declared
    /// by `expected_ty` (unknown existence),  if `expected_ty` declares a field not in the value
    /// map, or if `expected_ty` is not a record type.
    fn try_into_concrete_map(&self, expected_ty: &Type) -> Option<BTreeMap<SmolStr, Value>> {
        let Type::Record {
        ...
        // Now we try to convert all attribute values to concrete
        self.attrs()
            .filter_map(|(k, v)| {
    ...
    pub(crate) fn try_into_value(&self, expected_ty: &Type) -> Option<Value> {
        self.try_into_concrete_map(expected_ty)
    ...
    /// - `Value(v)`: the concrete record must have the key with a consistent value
    /// - `Exists`: the concrete record must have the key with some value
    /// - `Absent`: the concrete record must _not_ have the key
    ...
    pub(crate) fn to_residual_with_expr(self, ty: Type, parent_expr: Residual) -> Residual {
        if let Some(concrete) = self.try_into_value(&ty) {
...
impl<V> PartialAttribute<V> {
    ...
    pub(crate) fn from_declared(declared: Option<&AttributeType>) -> Self {
        match declared {
            // Declared as required, so we can assume it must exist.
...
impl PartialValue {
    ...
    /// attributes must be present in a record concrete value, but it is not used to typecheck the
    ...
    /// Convert to a `Residual`.
    ///
    /// `parent_expr` is the residual expression that produced this value (e.g., `Document::"doc".meta`).
    /// It becomes the `base` expression which will be used the target to resolve attribute values
    /// given more complete entity data later.
    pub(crate) fn to_residual_with_expr(self, ty: Type, parent_expr: Residual) -> Residual {
        match self {
            PartialValue::Record(rec) => rec.to_residual_with_expr(ty, parent_expr),
            // All non-record values have no partial information
            PartialValue::Lit(lit) => Residual::Concrete {
                value: normalize_ext_value(Value::new(lit, None)),
                ty,
            },
    ...
    pub fn all_literal_uids(&self) -> HashSet<EntityUID> {
        match self {
            PartialValue::Lit(Literal::EntityUID(euid)) => HashSet::from([euid.as_ref().clone()]),
            PartialValue::Lit(_) | PartialValue::ExtensionValue(_) => HashSet::new(),
            PartialValue::Set(set) => set.iter().flat_map(Value::all_literal_uids).collect(),
            PartialValue::Record(rec) => rec
                .attrs()
                .filter_map(|(_, a)| a.as_value())
