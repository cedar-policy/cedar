...
use crate::{
    ...
    RawUncheckedName, SchemaFragment, ValidationMode,
};
...
fn contains_typecheck_literals_false() {
    let schema: NamespaceDefinition<RawUncheckedName> = serde_json::from_value(json!(
    {
        "entityTypes": {},
        "actions": {
            "view_photo": { },
...
fn is_typecheck_fails() {
    let schema: NamespaceDefinition<RawUncheckedName> =
...
#[test]
fn is_typechecks() {
    let schema: SchemaFragment<RawUncheckedName> = serde_json::from_value(json!({
