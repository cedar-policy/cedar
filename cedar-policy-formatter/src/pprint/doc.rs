...
impl Doc for Node<Option<VariableDef>> {
    fn to_doc<'src>(&self, context: &mut Context<'_, 'src>) -> Option<RcDoc<'src>> {
        ...
        let start_comment =
            get_comment_at_start(self.loc.as_deref().map(|loc| loc.span), &mut context.tokens)?;
                ...
                RcDoc::line()
                    .append(add_comment(
                        ...
                        get_comment_after_end(
                            vd.variable.loc.as_deref().map(|loc| loc.span),
                            &mut context.tokens,
                        )?,
                        RcDoc::nil(),
                    ))
                    ...
                    .append(RcDoc::line().append(add_comment(
                        ...
                        get_comment_at_start(
                            entity_type.loc.as_deref().map(|loc| loc.span),
                            &mut context.tokens,
                        )?,
                        RcDoc::nil(),
                    )))
...
impl Doc for Node<Option<Cond>> {
    fn to_doc<'src>(&self, context: &mut Context<'_, 'src>) -> Option<RcDoc<'src>> {
        ...
        let lb_comment = get_comment_after_end(
            cond.cond.loc.as_deref().map(|loc| loc.span),
            &mut context.tokens,
        )?;
        let rb_comment =
            get_comment_at_end(self.loc.as_deref().map(|loc| loc.span), &mut context.tokens)?;
        ...
        Some(match cond.expr.as_ref() {
            Some(expr) => {
                let expr_leading_comment = get_leading_comment_at_start(
                    expr.loc.as_deref().map(|loc| loc.span),
                    &mut context.tokens,
                )?;
            ...
            Expr::Expr(expr_impl) => match expr_impl.expr.as_ref() {
                ExprData::If(c, t, e) => {
                    ...
                    let if_comment = get_comment_at_start(
                        self.loc.as_deref().map(|loc| loc.span),
                        &mut context.tokens,
                    )?;
                    let then_comment = get_comment_after_end(
                        c.loc.as_deref().map(|loc| loc.span),
                        &mut context.tokens,
                    )?;
                    let else_comment = get_comment_after_end(
                        t.loc.as_deref().map(|loc| loc.span),
                        &mut context.tokens,
                    )?;
                    Some(
...
impl Doc for Node<Option<Or>> {
    fn to_doc<'src>(&self, context: &mut Context<'_, 'src>) -> Option<RcDoc<'src>> {
        ...
        for e in es.iter().take(es.len() - 1) {
            let op_comment =
                get_comment_after_end(e.loc.as_deref().map(|loc| loc.span), &mut context.tokens)?;
            d = d
                .append(e.to_doc(context))
                .append(RcDoc::space())
                        ...
                        Some(
                            initial
                                ...
                                .append(add_comment(
                                    RcDoc::as_string(op),
                                    get_comment_after_end(
                                        initial.loc.as_deref().map(|loc| loc.span),
                                        &mut context.tokens,
                                    )?,
                                    RcDoc::nil(),
                                ))
            ...
            Relation::Has { target, field } => Some(
                target
                    ...
                    .append(add_comment(
                        RcDoc::text("has"),
                        get_comment_after_end(
                            target.loc.as_deref().map(|loc| loc.span),
                            &mut context.tokens,
                        )?,
                        RcDoc::nil(),
                    ))
                    ...
                    Some((
                        pair.0
                            ...
                            .append(add_comment(
                                op.to_doc(context)?,
                                get_comment_after_end(
                                    pair.1.loc.as_deref().map(|loc| loc.span),
                                    &mut context.tokens,
                                )?,
                                RcDoc::nil(),
                            ))
        ...
        if let Some(op) = e.op {
            match op {
                ...
                NegOp::Bang(n) | NegOp::Dash(n) => {
                    let sloc = self.loc.as_deref()?;
                    let eloc = e.item.loc.as_deref()?;
        ...
        Some(
            key_doc
                ...
                .append(add_comment(
                    RcDoc::text(":"),
                    get_comment_after_end(
                        e.0.loc.as_deref().map(|loc| loc.span),
                        &mut context.tokens,
                    )?,
                    RcDoc::nil(),
                ))
            ...
            Some(
                path.get(1..)?
                        ...
                        |pair, p| {
                            ...
                            Some((
                                d.append(add_comment(
                                    RcDoc::text("::"),
                                    get_comment_after_end(
                                        e.loc.as_deref().map(|loc| loc.span),
                                        &mut context.tokens,
                                    )?,
                                    RcDoc::nil(),
                                ))
                    ...
                    .append(add_comment(
                        RcDoc::text("::"),
                        get_comment_after_end(
                            path.last()?.loc.as_deref().map(|loc| loc.span),
                            &mut context.tokens,
                        )?,
                        RcDoc::nil(),
                    ))
            ...
            Ref::Uid { path, eid } => Some(
                path.to_doc(context)?
                    .append(add_comment(
                        RcDoc::text("::"),
                        get_comment_after_end(
                            path.loc.as_deref().map(|loc| loc.span),
                            &mut context.tokens,
                        )?,
                        RcDoc::nil(),
                    ))
        ...
        let val_doc = match annotation.value.as_ref() {
            Some(value) => {
                let lp_doc = add_comment(
                    RcDoc::text("("),
                    get_comment_after_end(
                        annotation.key.loc.as_deref().map(|loc| loc.span),
                        &mut context.tokens,
                    )?,
                    RcDoc::nil(),
                );
...
impl Doc for Node<Option<Policy>> {
    fn to_doc<'src>(&self, context: &mut Context<'_, 'src>) -> Option<RcDoc<'src>> {
        ...
        let eff_leading_comment = get_leading_comment_at_start(
            policy.effect.loc.as_deref().map(|loc| loc.span),
            &mut context.tokens,
        )?;
        ...
        let vars_doc = if vars.get(0..3)?.iter().all(|v| {
            ...
            principal_doc
                .append(add_comment(
                    RcDoc::text(","),
                    get_comment_after_end(
                        vars.first()?.loc.as_deref().map(|loc| loc.span),
                        &mut context.tokens,
                    )?,
                    RcDoc::space(),
                ))
                ...
                .append(add_comment(
                    RcDoc::text(","),
                    get_comment_after_end(
                        vars.get(1)?.loc.as_deref().map(|loc| loc.span),
                        &mut context.tokens,
                    )?,
                    RcDoc::space(),
                ))
        ...
        Some(
            anno_doc
                ...
                .append(add_comment(
                    RcDoc::text(")"),
                    get_comment_after_end(
                        vars.get(2)?.loc.as_deref().map(|loc| loc.span),
                        &mut context.tokens,
                    )?,
                    if conds.is_empty() {
                        RcDoc::nil()
