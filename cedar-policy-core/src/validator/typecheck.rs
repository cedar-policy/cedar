...
impl<'a> SingleEnvTypechecker<'a> {
                    ...
                    Some(typ_actual) => {
                        match Type::lookup_attribute_type(self.schema, typ_actual, attr) {
                            Some(AttributeType {
                                ...
                                let target_is_record = matches!(typ_actual, Type::Record { .. });
                                ...
                                let in_prior_capability = prior_capability
                                    .contains(&Capability::new_attribute(expr, attr.clone()));
                                let type_of_has = if target_is_record || in_prior_capability {
    ...
    ///
    /// If `kind` does not have a `tags` declaration, this returns with the
    /// empty set.
    ///
    ...
    fn check_action_in_type(&self, lhs_ety: &EntityType, rhs_ty: &Type) -> bool {
        ...
        match rhs_ty {
            ...
            Type::Entity(EntityKind::Entity(rhs_etys)) => rhs_etys
                .iter()
                ...
                match (lhs_as_euid_lit.expr_kind(), rhs_as_euid_lit.expr_kind()) {
                    ...
                    _ => {
                        match (lhs_expr.data(), rhs_expr.data()) {
                            ...
                            (Some(Type::Entity(EntityKind::Entity(lhs_etys))), Some(rhs_ty))
                                if lhs_etys
                                    .iter()
