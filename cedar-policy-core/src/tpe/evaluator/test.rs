...
/// A record attribute about which nothing is known beyond its declared type.
pub(crate) fn unknown_record() -> AttrState {
    AttrState::PartialRecord(PartialRecord::new())
}
...
/// `e has a.b.c` should reduce exactly like `e has a && e.a has b && e.a.b has c`.
mod ext_has_attr_partial_attr_tests {
    use super::*;
    ...
    test_env!(
        ...
        entity Document { meta: { inner?: { leaf?: String } }, link?: Leaf };
        action Read appliesTo { principal: User, resource: Document, context: {} };
        "#,
        r#"User::"alice""#,
        "Read",
    ...
    /// Evaluate `cond` against a `Document::"doc"` whose `meta` is `meta`, `link` is `link`, and
    /// with `leaves` as the extra entities in the store.
    fn eval(
        meta: PA,
        link: PA,
        leaves: impl IntoIterator<Item = PartialEntity>,
        cond: &str,
    ) -> String {
        let (schema, req) = setup();
        let doc = entity(
    ...
    /// As [`eval`], with `link` absent and no extra entities.
    fn eval_meta(meta: PA, cond: &str) -> String {
        eval(meta, PA::Absent, [], cond)
    }

    #[test]
    fn nested_record_chain() {
        let cond = "resource has meta.inner.leaf";
        // Every step known.
        assert_snapshot!(
            eval_meta(record([("inner", record([("leaf", present("x"))]))]), cond),
            @"true"
        );
        // `leaf` exists but its value is unknown: `has` is still decided.
        assert_snapshot!(
            eval_meta(record([("inner", record([("leaf", PA::Present)]))]), cond),
            @"true"
        );
        // A `false` anywhere in the chain short-circuits.
        assert_snapshot!(
            eval_meta(record([("inner", record([("leaf", PA::Absent)]))]), cond),
            @"false"
        );
        assert_snapshot!(
            eval_meta(record([("inner", PA::Absent)]), cond),
            @"false"
        );
        // `inner` exists but is unknown, so only the `leaf` step is left over.
        assert_snapshot!(
            eval_meta(record([("inner", PA::Present)]), cond),
            @r#"Document::"doc".meta.inner has leaf"#
        );
        // Nothing is known about `inner`, so the whole chain is left over.
        assert_snapshot!(
            eval_meta(unknown_record(), cond),
            @r#"Document::"doc".meta has inner.leaf"#
        );
    }

    #[test]
    fn entity_chain() {
        let cond = "resource has link.value";
        let leaf = |value: PA| entity("Leaf", "l", Some(rec([("value", value)])), None);
        assert_snapshot!(
            eval(unknown_record(), present_uid("Leaf", "l"), [leaf(present("v"))], cond),
            @"true"
        );
        assert_snapshot!(
            eval(unknown_record(), present_uid("Leaf", "l"), [leaf(PA::Absent)], cond),
            @"false"
        );
        // `link` is absent, so we never look at `Leaf::"l"`.
        assert_snapshot!(
            eval(unknown_record(), PA::Absent, [leaf(present("v"))], cond),
            @"false"
        );
        // `Leaf::"l"` is not in the store, so its attributes are unknown.
        assert_snapshot!(
            eval(unknown_record(), present_uid("Leaf", "l"), [], cond),
            @r#"Leaf::"l" has value"#
        );
        // `link` exists but which entity it is is unknown.
        assert_snapshot!(
            eval(unknown_record(), PA::Present, [leaf(present("v"))], cond),
            @r#"Document::"doc".link has value"#
        );
    }

    #[test]
    fn matches_desugared_has_chain() {
        let ext = "resource has meta.inner.leaf";
        let desugared =
            "resource has meta && resource.meta has inner && resource.meta.inner has leaf";
        for meta in [
            record([("inner", record([("leaf", present("x"))]))]),
            record([("inner", record([("leaf", PA::Present)]))]),
            record([("inner", record([("leaf", PA::Absent)]))]),
            record([("inner", PA::Absent)]),
            record([("inner", PA::Present)]),
            unknown_record(),
        ] {
            let ext_residual = eval_meta(meta.clone(), ext);
            let desugared_residual = eval_meta(meta.clone(), desugared);
            // The desugared form spells out the steps it has already decided, so compare only
            // the conclusions that are fully decided.
            if ext_residual == "true" || ext_residual == "false" {
                assert_eq!(ext_residual, desugared_residual, "for meta {meta:?}");
            }
        }
    }
}
...
mod iterated_eval_tests {
    ...
    fn eval_twice(
        ...
        let rps = tpe::residual_policies(&req, &policies, &schema, &eval).unwrap();
        let first = rps.first().unwrap().as_residual();
...
mod export_tests {
    ...
    fn export_is_idempotent() {
        ...
        let interp = |cond: &str| {
            ...
            (
                Expr::from(first.clone()).to_string(),
