...
/// Controls how the `(reset)` that starts each solver query is written into the
/// SMTLib script.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum ResetMode {
    /// Emit the `(reset)` command, clearing assertions from
    /// previous query on the same solver. Required
    /// for any `Solver` that reuses one solver session across queries, such as
    /// [`crate::solver::LocalSolver`].
    #[default]
    Emit,
    /// Emit `(reset)` as an SMTLib comment (`; (reset)`).
    ///
    /// This is appropriate only for `Solver` implementations that send each
    /// query as a standalone script (so there is no leftover assertions to clear).
    Comment,
}
...
impl<S: Solver> SymCompiler<S> {
    /// Emits the `(reset)` that starts a query, as dictated by `reset_mode`.
    async fn emit_reset(&mut self) -> Result<()> {
        let script = self.solver.smtlib_input();
        match self.reset_mode {
            ResetMode::Emit => script.reset().await,
            ResetMode::Comment => script.comment("(reset)").await,
        }
    ...
    /// call resets the solver, unless the [`ResetMode`] says otherwise.
    ...
    /// resets the solver, unless the [`ResetMode`] says otherwise.
    ...
    /// with respect to `symenv` (see notes in the Lean). This call resets the
    /// solver, unless the [`ResetMode`] says otherwise.
    ///
