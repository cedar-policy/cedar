...
pub trait ExprBuilder: Clone {
    ...
    fn extended_has_attr_arc(self, expr: Arc<Self::Expr>, attrs: &NonEmpty<SmolStr>) -> Self::Expr {
        ...
        let has_expr = Self::new()
            ...
            .has_attr_arc(expr.clone(), first.to_owned());
        let get_expr = Self::new()
            .with_maybe_source_loc(self.loc())
            .get_attr_arc(expr, first.to_owned());
        ...
        rest.iter()
            .fold(
                (has_expr, Arc::new(get_expr)),
                |(has_expr, get_expr), attr| {
                    (
                        Self::new().with_maybe_source_loc(self.loc()).and(
                            has_expr,
                            Self::new()
                                .with_maybe_source_loc(self.loc())
                                .has_attr_arc(get_expr.clone(), attr.to_owned()),
                        ),
                        Arc::new(
                            Self::new()
                                .with_maybe_source_loc(self.loc())
                                .get_attr_arc(get_expr, attr.to_owned()),
                        ),
                    )
                },
            )
