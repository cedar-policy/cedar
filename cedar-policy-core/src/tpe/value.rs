...
impl PartialRecord {
    ...
    pub fn from_concrete_map(map: &BTreeMap<SmolStr, Value>, ty: &Type) -> Self {
        let Type::Record {
            ...
            panic!("expected record value would have record type");
        };
        PartialRecord::from_attrs(attrs_tys.iter().map(|(k, aty)| {
    ...
    pub fn from_value(value: Value, ty: &Type) -> Self {
        match value.value {
            ...
            ValueKind::Record(attrs) => {
                PartialValue::Record(PartialRecord::from_concrete_map(attrs.as_ref(), ty))
            }
