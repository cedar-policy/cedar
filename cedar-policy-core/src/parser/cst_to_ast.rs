...
impl Node<Option<cst::Policies>> {
    ...
    pub fn with_generated_policyids(
        &self,
    ) -> Result<impl Iterator<Item = (ast::PolicyID, &Node<Option<cst::Policy>>)>, err::ParseErrors>
    {
    ...
    pub fn to_policyset(&self) -> Result<ast::PolicySet, err::ParseErrors> {
        let mut pset = ast::PolicySet::new();
        let mut all_errs: Vec<err::ParseErrors> = vec![];
                    ...
                    if let Err(e) = pset.add_template(template) {
                        match e {
                            PolicySetError::Occupied { id } => {
                                all_errs.push(err::ParseErrors::singleton(
...
impl Node<Option<cst::Policy>> {
    ...
    pub fn to_policy_or_template(
        &self,
        id: ast::PolicyID,
    ) -> Result<Either<ast::StaticPolicy, ast::Template>, err::ParseErrors> {
        let mut errs = vec![];
        if let Some(t) = self.to_policy_template(id, &mut errs) {
            if t.slots().count() == 0 {
