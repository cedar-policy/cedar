...
mod test {
    ...
    fn parse_residual(expr_str: &str) -> Residual {
        ...
        let schema = ValidatorSchema::from_cedarschema_str(r#"
            entity User { foo: Bool, str: String, num: Long, period: __cedar::duration, set: Set<String> } tags String;
            entity Document; 
    ...
    fn test_can_error_assuming_well_formed() {
        assert_eq!(
            parse_residual(r#"principal has foo && {a: true, b: false}["a"]"#)
                .can_error_assuming_well_formed(),
            false
        );
        assert_eq!(
            parse_residual(r#"principal is User || User::"jane".str like "jane-*""#)
                .can_error_assuming_well_formed(),
            false
        );
        assert_eq!(
        ...
        assert_eq!(
            parse_residual(r#"!principal.set.isEmpty() && (principal.set.contains("foo") || principal.set.containsAll(["foo", "bar"]) || principal.set.containsAny(["foo", "bar"]))"#).can_error_assuming_well_formed(),
            false
        );
        assert_eq!(
