...
impl<V> AttrState<V> {
    ...
    pub(crate) fn try_decide_exists(&self) -> Option<bool> {
        match self {
            AttrState::Value(_) | AttrState::PartialRecord(_) | AttrState::Present => Some(true),
            AttrState::Absent => Some(false),
            AttrState::Unknown => None,
        }
    }
