        ...
        UnaryOp::Neg => {
            ...
            match i.checked_neg() {
                ...
                None => Err(IntegerOverflowError::UnaryOp(UnaryOpOverflowError {
                    op,
                    arg,
                    source_loc: loc.as_deref().map(|loc| Box::new(loc.clone())),
                })
                .into()),
            }
        }
...
impl<'e> RestrictedEvaluator<'e> {
    ...
    pub fn partial_interpret(&self, expr: BorrowedRestrictedExpr<'_>) -> Result<PartialValue> {
        ...
        res.map(|pval| pval.with_maybe_source_loc(expr.source_loc().as_deref().map(|loc| Box::new(loc.clone()))))
            .map_err(|err| match err.source_loc() {
                None => err.with_maybe_source_loc(expr.source_loc().as_deref().map(|loc| Box::new(loc.clone()))),
                Some(_) => err,
            })
    }
    ...
    fn partial_interpret_internal(&self, expr: BorrowedRestrictedExpr<'_>) -> Result<PartialValue> {
        match expr.as_ref().expr_kind() {
            ...
            ExprKind::Set(items) => {
                ...
                match split(vals) {
                    Either::Left(values) => Ok(Value::set(values, expr.source_loc().as_deref().map(|loc| Box::new(loc.clone()))).into()),
            ...
            ExprKind::Record(map) => {
                ...
                match split(attrs) {
                    Either::Left(values) => Ok(Value::record(names.into_iter().zip(values), expr.source_loc().as_deref().map(|loc| Box::new(loc.clone()))).into()),
    ...
    fn partial_interpret_internal(&self, expr: &Expr, slots: &SlotEnv) -> Result<PartialValue> {
        ...
        match expr.expr_kind() {
            ...
            ExprKind::Slot(id) => slots
                .get(id)
                .ok_or_else(|| err::EvaluationError::unlinked_slot(*id, loc.as_deref().map(|loc| Box::new(loc.clone()))))
                                ...
                                match self.entities.entity(uid) {
                                    Dereference::NoSuchEntity => {
                                        ...
                                        Err(EvaluationError::entity_does_not_exist(
                                            Arc::new(uid.clone()),
                                            arg1.source_loc().as_deref().map(|loc| Box::new(loc.clone())),
                                        ))
                                    }
                                    Dereference::Residual(r) => Ok(PartialValue::Residual(
                                    ...
                                    Dereference::Data(entity) => entity
                                        ...
                                        .ok_or_else(|| {
                                            EvaluationError::entity_tag_does_not_exist(
                                                ...
                                                entity.tags_len(),
                                                loc.as_deref().map(|loc| Box::new(loc.clone())), // intentionally using the location of the entire `GetTag` expression
                                            )
                                        })
                                        .cloned(),
                                }
            ...
            ExprKind::Set(items) => {
                let vals = items
                    ...
                    .collect::<Result<Vec<_>>>()?;
                match split(vals) {
                    Either::Left(vals) => Ok(Value::set(vals, loc.as_deref().map(|loc| Box::new(loc.clone()))).into()),
            ...
            ExprKind::Record(map) => {
                ...
                match split(evalled) {
                    Either::Left(vals) => {
                        Ok(Value::record(names.into_iter().zip(vals), loc.as_deref().map(|loc| Box::new(loc.clone()))).into())
                    }
                    Either::Right(rs) => {
                        // PANIC SAFETY: can't have a duplicate key here because `names` is the set of keys of the input `BTreeMap`
                        #[allow(clippy::expect_used)]
