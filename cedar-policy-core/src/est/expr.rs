...
use crate::ast::{self, CallStyle};
use crate::entities::{FnAndArg, JSONValue, JsonDeserializationError, TypeAndId};
...
fn display_jsonvalue(f: &mut std::fmt::Formatter<'_>, v: &JSONValue) -> std::fmt::Result {
    match v {
        // Add parentheses around negative numeric literals otherwise
        // round-tripping fuzzer fails for expressions like `(-1)["a"]`.
        JSONValue::Long(n) if *n < 0 => write!(f, "({n})"),
        JSONValue::Long(n) => write!(f, "{n}"),
        JSONValue::Bool(b) => write!(f, "{b}"),
        JSONValue::String(s) => write!(f, "\"{}\"", s.escape_debug()),
        JSONValue::EntityEscape { __entity } => match ast::EntityUID::try_from(__entity.clone()) {
            Ok(euid) => write!(f, "{euid}"),
            Err(e) => write!(f, "(invalid entity uid: {})", e),
        },
        JSONValue::ExprEscape { __expr } => write!(f, "({__expr})"),
        JSONValue::ExtnEscape {
...
impl std::fmt::Display for ExprNoExt {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match &self {
            ExprNoExt::Value(v) => display_jsonvalue(f, v),
...
impl std::fmt::Display for ExtFuncCall {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let Some((fn_name, args)) = self.call.iter().next() else { unreachable!("invariant violated: empty ExtFuncCall") };
        // search for the name and callstyle
        ...
        match (style, args.iter().next()) {
            (Some(CallStyle::MethodStyle), Some(receiver)) => {
                write!(
                    f,
                    "{}.{}({})",
