...
pub fn fuzzy_search(key: &str, lst: &[impl AsRef<str>]) -> Option<SmolStr> {
    if key.is_empty() || lst.is_empty() {
        None
    } else {
        let (_, result) = lst.iter().fold((std::usize::MAX, ""), |acc, word| {
...
pub mod test {
    ...
    fn test_match1() {
        ...
        assert_eq!(x, Some("User::Alice".into()));
    }
    ...
    fn test_match2() {
        ...
        let x = fuzzy_search(word1, &words);
        assert_eq!(x, Some("principal".into()));
    }
