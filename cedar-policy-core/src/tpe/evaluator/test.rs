...
mod partial_attr_tests {
    ...
    fn setup() -> (ValidatorSchema, PartialRequest) {
        ...
        let req = request(r#"User::"alice""#, "Read", r#"Document::"doc""#, &schema);
        (schema, req)
    }
...
mod record_literal_tests {
    ...
    fn test_non_erroring_residual_sibling_still_blocks() {
        ...
        let req = request("User", "Read", r#"Document::"doc""#, &schema);
...
mod iterated_eval_tests {
    ...
    fn eval_twice(
        ...
        let render = |r: &Residual| crate::ast::Expr::from(r.clone()).to_string();

        let ents1 = PartialEntities::from_entities(round1.into_iter(), &schema).unwrap();
        let residuals = crate::tpe::policy_residual_map(&req, &policies, &schema).unwrap();
